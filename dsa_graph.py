...
class State(TypedDict):
    ...
    hint: str
    explanation: str # the above two will be populted by the hint generator agent
...
def hint_node(state: State):
    """ this node will generate a hint for the user to use to solve the question """
    ...
    # invoke the hint generator
    hint = hint_gen.generate_hint(topic , title , description , test_cases)
    print(f"\n The hint generated by the agent is : \n\n {hint}\n\n And the explanation is : \n\n {hint['explanation']}")
    # update the state
    return {"hint": hint['hint'] , "explanation": hint['explanation']}
...
def genetate_dsa_graph():
    ...
    # add an edge from hint node back to router
    graph.add_node("Hint Node", hint_node)
    graph.add_edge("Hint Node", "Human Review")
...
graph.invoke({"topic": "BFS algorithm and its applications"})
